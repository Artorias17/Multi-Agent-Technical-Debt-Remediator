...
def allocate_proportional(counts: dict, total: int) -> dict:
    ...
    for k, n in counts.items():
        allocation[k] = max(1, round(n / pool_total * total))
...
def sample_stratum(pool: list, n: int, file_counts: dict) -> list:
    ...
    for issue in pool:
        ...
        if file_counts[comp] < MAX_PER_FILE:
...
def main():
    parser = argparse.ArgumentParser(description="Hierarchical stratified sample from SonarQube report")
    ...
    type_pool_sizes = {t: sum(len(v) for v in by_type_sev[t].values()) for t in types_present}
    ...
    if diff != 0:
        largest = max(type_allocation, key=type_allocation.get)
    ...
    for t in types_present:
        ...
        for s in SEVERITY_ORDER:
            if s in sev_allocation:
                print(f"    {s:10s}: {sev_allocation[s]:3d}  (pool: {sev_pool_sizes[s]})")
    ...
    ext_dist = Counter(Path(i.get("component", "").split(":")[-1]).suffix.lower() for i in sampled)
    ...
    for t in TYPE_ORDER:
        if t in type_dist:
            print(f"  {t:15s}: {type_dist[t]:3d}  ({type_dist[t]/len(sampled)*100:.1f}%)")
    ...
    output_path = args.output or f"data/sonar_report_{report.get('project_id', 'sample')}_stratified.json"
