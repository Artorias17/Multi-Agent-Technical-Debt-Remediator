...
def vcs_commit_chunk(state: PipelineState) -> dict:
    ...
    issue_lines = []
    for item in file_entries:
        for issue in item["patch"].get("issues", []):
            rule = issue.get("rule", "?")
            sev = issue.get("severity", "?")
            line = issue.get("line") or (issue.get("textRange") or {}).get("startLine", "?")
            issue_lines.append(f"- {rule} ({sev}): line {line}")

    body = ""
    if issue_lines:
        body = "Issues resolved:\n" + "\n".join(issue_lines) + "\n\n"
    body += "Generated by TD Agent. Review before merging."

    if changelog:
        msg = changelog + "\n\n" + body
    else:
        msg = _build_commit_message(rule=rule_str, file=Path(file_path).name, changelog=body)
